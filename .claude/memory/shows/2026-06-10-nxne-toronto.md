---
event: NXNE (Billboard show), Toronto
date: 2026-06-10
event_id: TBD
stage: Settlement
updated: 2026-06-10
---
# NXNE (Billboard show), Toronto — 2026-06-10

bbno$ Billboard-presented NXNE (North by Northeast) show, Toronto, Wed Jun 10. Venue = Drake Underground. SHOW IS TODAY.

## Booking / Confirm
- Confirmed date Jun 10. Venue: Drake Underground, Toronto.

## Advance (tech, hospitality, production, day sheet, contacts)
- PR: Jess Manno (jess@cancelcommunications.com, Cancel Communications); CC Matthew + Brandon (cancelcommunications.com). Mgmt Sandro Lipari added Davon as TM to advise on editor tickets + ROS.

## Day-of (schedule/ROS, guest list, settlement prep)
- Editor/press tickets: 06-10 Davon approved final editor list, all available for box-office pickup: Sydney Brasil (Exclaim!) +1, Sidra Jafri (NOWToronto), Gareth Jones (You Are Hear / bbno$ radio plugger). Closed for now; Jess may send more names day-of.

## Settlement / Post (payment, reconciliation)
- LED wall (PixelIt Rent, Mike Savvin, pixelit-rent.ca): Dan Nudelman confirmed hire 06-08; invoice to BBNO Shows Inc C/O Dan Nudelman, Vancouver. Power = 5x 15A Edison circuits; stair load-in, elevator coordinated ~1pm; PixelIt crew load-in 1:00pm. REIMBURSEMENT RESOLVED 06-10: paid out of pocket, Dan forwards final invoice to Billboard who add it to the post-show settlement. Davon approved; Tony Yacowar (DMCL) handling payment.

## Open items / next actions
- [ ] Reconcile LED-wall invoice into Billboard settlement (Dan forwarding final invoice to Billboard).

## Sources (Gmail thread ids + last seen)
- 19ea81229d4fb763 — EDITOR TICKETS FOR BILLBOARD NXNE SHOW (editors approved, box-office pickup) — 2026-06-10
- 19eadee5729eb7bb — FW: LED Wall Inquiry - Drake Underground (reimbursement via Billboard settlement; load-in 1pm) — 2026-06-10
