...
- [2026-04-22 08:00 UTC] https://platform.claude.com/docs/en/about-claude/models/migration-guide — Opus 4.7 introduces hard API breaking changes vs 4.6: (1) setting temperature, top_p, or top_k to any non-default value now returns HTTP 400 — omit these params entirely; (2) extended thinking budget_tokens mode removed — `thinking: {type: "enabled", budget_tokens: N}` returns 400, replace with `thinking: {type: "adaptive"}`; (3) new tokenizer uses up to 35% more tokens than 4.6 (varies by content) — /v1/messages/count_tokens returns different values. Critical: any dos-platform or api/intel.js code using temperature/top_p/top_k or explicit thinking budgets will break when default switches to 4.7 on Apr 23.

- [2026-04-22 08:00 UTC] https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7 — Opus 4.7 adds two new API parameters not in 4.6: (1) effort level — string enum `standard | high | maximum`, controls reasoning depth before responding; (2) task budgets — set per-subtask token limits within agentic workflows to prevent runaway reasoning on low-value steps. Both are additive (no breakage if omitted), but relevant for Phase 4 Claude agent architecture in dos-platform.

- [2026-04-22 08:00 UTC] https://nextjs.org/blog/next-16-1 — Next.js 16.1 (released between 16.0 stable and 16.2): Turbopack file-system caching now stable and on by default — dev server restart up to 14x faster on large projects (cold: 15s → cached: 1.1s on large Vercel internal app); `next upgrade` CLI command added for one-command version upgrades; Next.js DevTools MCP server ships `get_routes` tool exposing full route list to agents; core package 20MB lighter (faster CI installs). All relevant to Dane's Jun/Jul build window on dos-platform.

- [2026-04-22 08:00 UTC] https://nextjs.org/blog/next-16-2-ai — Next.js 16.2 (March 18, 2026) AI-assisted development features: browser errors now forwarded to terminal by default during `next dev`, agents (Claude Code) can read and debug runtime errors without opening a browser; agents can inspect running apps from terminal. Memory had "Next.js 16.2 shipped March 18, 2026" but not the specific features — this adds content to that entry.

- [2026-04-22 08:00 UTC] https://supabase.com/changelog — Supabase April 2026 Developer Update: AI-powered table filters in Studio dashboard — describe desired filter in natural language, dashboard auto-applies correct Postgres filter conditions; Schema Visualiser improved with clickable relation lines, table/column context actions, and popovers between connected tables. Both useful during Phase 0 schema audit with Josh; Schema Visualiser in particular replaces manual diagramming.
