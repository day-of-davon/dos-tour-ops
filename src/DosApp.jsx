...
function FinLedger(){
  ...
  const rows=useMemo(()=>{
    ...
    Object.entries(finance).forEach(([date,fin])=>{
      ...
      (fin.flightExpenses||[]).forEach(fe=>{
        ...
        out.push({id:fe.flightId||`fe_${date}_${Math.random()}`,date,show:showLabel,cat:"Flight",desc:fe.label||"",payee:(fe.pax||[]).join(", ")||"—",amount:parseFloat(fe.amount||0),currency:fe.currency||"USD",status:"confirmed",ref:fe.carrier||"",bookedDate:fe.bookedDate||"",paidDate:fe.paidDate||"",_src:{type:"flightExpense",date,srcId:fe.flightId}});
      });
      (fin.payouts||[]).forEach(p=>{
        out.push({id:p.id||`po_${date}_${Math.random()}`,date,show:showLabel,cat:"Payout",desc:`${p.dept||""}${p.role?` · ${p.role}`:""}`,payee:p.name||"—",amount:parseFloat(p.amount||0),currency:p.currency||"USD",status:p.status||"pending",ref:p.method||"",bookedDate:p.bookedDate||"",paidDate:p.paidDate||"",_src:{type:"payout",date,srcId:p.id}});
      });
      (fin.ledgerEntries||[]).forEach(le=>{
        if(!le.amount&&le.amount!==0)return;
        out.push({id:le.id||`le_${date}_${Math.random()}`,date:le.date||date,show:showLabel,cat:"Hotel",desc:le.description||"",payee:le.vendor||"—",amount:parseFloat(le.amount||0),currency:le.currency||"USD",status:"confirmed",ref:le.source||"",bookedDate:le.bookedDate||"",paidDate:le.paidDate||"",_src:{type:"ledgerEntry",date,srcId:le.id}});
      });
      ...
      if(fin.settlementAmount&&parseFloat(fin.settlementAmount)>0&&!hasEventForLegacy){
        out.push({id:`sa_${date}`,date,show:showLabel,cat:"Settlement",desc:"Settlement payment",payee:"—",amount:parseFloat(fin.settlementAmount),currency:"USD",status:fin.stages?.payment_initiated?"confirmed":"pending",ref:fin.wireRef||"",bookedDate:"",paidDate:"",_src:{type:"legacySettlement",date,srcId:null}});
      }
      (fin.events||[]).forEach(ev=>{
        if(!ev||!ev.amount)return;
        const cat=(FIN_EVENT_TYPES.find(t=>t.id===ev.type)?.l)||"Event";
        out.push({id:ev.id,date:ev.actualDate||ev.expectedDate||date,show:showLabel,cat,desc:ev.note||cat,payee:"—",amount:parseFloat(ev.amount)||0,currency:ev.currency||"USD",status:ev.status||"pending",ref:ev.ref||"",bookedDate:ev.bookedDate||"",paidDate:ev.paidDate||"",_src:{type:"event",date,srcId:ev.id}});
      });
    });
  ...
  const commit=()=>{
    ...
    if(type==="confirmedFlight"){
      ...
      const FK={desc:"note",date:"actualDate"};
      ...
      const FK={amount:"settlementAmount",ref:"wireRef"};
      ...
      {sorted.length===0?(
        ...
        <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:10,overflow:"hidden"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr>{[["date","Date"],["bookedDate","Booked"],["paidDate","Paid"],["show","Show"],["cat","Category"],["payee","Payee"],["desc","Description"],["amount","Amount"],["currency","Curr"],["status","Status"],["ref","Ref"]].map(([col,label])=>th(label,col))}</tr></thead>
            <tbody>
              {sorted.map((r,i)=>{
                ...
                return(
                  <tr key={r.id} style={{borderBottom:"1px solid var(--card-3)",background:bg}}>
                    ...
                    {ecell(r,"paidDate",r.paidDate,{...d0,fontFamily:MN})}
